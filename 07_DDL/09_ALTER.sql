/*
    <ALTER>
    오라클 데이터베이스 객체(테이블, 인덱스, 뷰 등)을 수정하는 구문이다.
*/
-- 실습에 사용할 테이블 생성
...
-- 변경하려는 자료형의 크기보다 이미 큰값이 존재하면 에러 발생
--ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(2);
--ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10);

-- DEPT_COPY 테이블에 CNAME 컬럼의 데이터 타입을 NUMBER로 변경
ALTER TABLE DEPT_COPY MODIFY CNAME NUMBER; -- 컬럼 값들이 비어있기 때문에(NULL) 데이터 타입 변경 가능
ALTER TABLE DEPT_COPY MODIFY LNAME NUMBER; -- 값들이 비어있지 않아서 변경 불가능 (에러 발생)

-- DEPT_COPY 테이블에 LNAME 컬럼의 기본값을 미국으로 변경
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT '미국';
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT NULL; -- 기본값(DEFAULT) 삭제

-- DEPT_COPY 테이블에 DEPT_TITLE 컬럼의 데이터 타입을 VARCHAR2(40)로, LOCATION_ID 컬럼의 데이터 타입을 VARCHAR2(2)로,
...
ALTER TABLE DEPT_COPY DROP COLUMN CNAME; -- 에러발생
...
*/
-- DEPT_COPY 테이블의 DEPT_TITLE 컬럼명을 LOCATION_NAME으로 변경
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_TI_UN UNIQUE(DEPT_TITLE);
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT EM_CO_FK FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT(DEPT_ID);
ALTER TABLE EMPLOYEE ADD CONSTRAINT EM_JO_FK FOREIGN KEY(JOB_CODE) REFERENCES JOB(JOB_CODE);

-- MEMBER, MEMBER_GRADE 테이블 생성 후 제약조건을 추가
...
-- MEMBER 테이블 생성 후 PK, FK, NOT NULL, UNIQUE, CHECK 재약조건을 추가
CREATE TABLE MEMBER (
    NO NUMBER,
    ID VARCHAR2(20),
...
ALTER TABLE MEMBER ADD CONSTRAINT MEMBER_GRADE_ID_FK FOREIGN KEY(GRADE_ID) REFERENCES MEMBER_GRADE(GRADE_CODE) ON DELETE CASCADE;
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_TI_UN;
...
*/
-- DEPT_COPY 테이블에서 SYS_C008493 제약조건명을 DEPT_ID_NN 로 변경
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C008493 TO DEPT_ID_NN;

-- DEPT_COPY 테이블에서 SYS_C008494 제약조건명을 LOCATION_ID_NN 로 변경
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C008494 TO LOCATION_ID_NN;
...
-- MEMBER_GRADE 테이블 삭제 
-- 참조되고 있는 부모 테이블을 함부로 삭제가 되지 않는다 
DROP TABLE MEMBER_GRADE;
